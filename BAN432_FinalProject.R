...
# working directory is not set since this file was inside an Rproject file
# please adjust directories for saving data and loading data to your personal WD
...
library(slam) # dtm wrangling
library(tidytext) # functions for cleaning text
library(lubridate) # date transformation functions
library(stringr) # string manipulation functions
library(topicmodels) # LDA topic modelling
library(wordcloud) # wordcloud visualizations
library(sentimentr) # sentiment analysis
library(lexicon) # Dictionaries for sentiment analysis (LM)
library(stringi) # string manipulation
library(tokenizers) # standard tokenizer
...
# load all earning calls in dataframe (call-column then contains large list per row)
...
# Make industry column by checking for sector information in 12 $included-elements
...
# add columns to carry through for further analysis
# year, month, quarter, year-month, year-quarter
data.raw <- data.raw %>% 
...
# We decide that dropping 3648 earnings calls is not satisfactory and download industry
# classification online (we have stock symbol information in $included-element 2)
...
# for the final analysis, this is commented out, since we conduct the
# analysis with all 7310 earnings calls
# set.seed(1234)
...
# Note: we were not able to conduct analysis split by QA and executive presentation
# We still include this code, since it contains an important outlook for our paper
data.pre <- data.raw %>%
  ...
  # select the columns that will be important for further analysis
  select(symbol,
         company_name,
         id,
         date,
...
# remove company names and all words contained in company names
...
# bind all important columns that we want to analyze by
# This dataframe will serve as the main analytical source for Task 1
...
# we look at how they talk about it by analyzing the surrounding context of the keywords
...
custom_stop_kwic <- c("aiaiai", "openai", "chatgpt")
...
filename <- "plots/wc_kwic_chatgpt_2.png"
...
# analyse the sentiment of the KWIC
# most frequent negative and positive tokens in KWIC
plot_kwic_common_posneg <- pre_post_freq %>% 
  left_join(tibble(neg_word = DictionaryLM$negative, neg_scale = -1), by = c("tokens" = "neg_word")) %>% 
  left_join(tibble(pos_word = DictionaryLM$positive, pos_scale = 1), by = c("tokens" = "pos_word")) %>% 
  mutate(neg_pos = ifelse(!is.na(neg_scale), neg_scale, pos_scale)) %>% 
  filter(!is.na(neg_pos)) %>% 
  filter(!tokens %in% c("question", "questions")) %>% 
  group_by(neg_pos) %>% 
  slice_max(order_by = n, n = 10) %>% 
  ggplot(aes(x = reorder(tokens, (n * neg_pos)), y = (n * neg_pos), fill = factor(neg_pos))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("1" = "blue", "-1" = "red")) +
  labs(title = "Top Most Common Positive and Negative Terms",
       x = "Token",
       y = "Frequency",
       fill = "") +
  theme_minimal() +
...
png("plots/plot_kwic_common_posneg.png", width = 1200, height = 800)
plot_kwic_common_posneg
dev.off()
...
# note that we take the full content, not the cleaned content (by sentence)
...
# All negative sentences that contain AI-related words
negative_sentences <- ai_sentiment_filtered %>% 
  filter(sentiment < 0) %>% 
  arrange(sentiment) %>% 
  pull(sentence)

# save as appendix to our report for further qualitative
# Analysis and interpretation of the sentences at a later point
# These negative sentences, when quickly screened and filtered with human
# intuition can already function as a nice summary of risks associated with AI
save(negative_sentences, file = "BAN432_FinalProject_HandIn/RQ2_negative_sentences.Rdata")


## Topic modelling for sentences that contain AI related words
# We try the topic modelling for
#   (1) unigrams
#   (2) bigrams
#   (3) trigrams

# (2) trying it with bigrams
# to use the sentences as input for bigram dtm, we clean the data
...
# (3) trying it with trigrams
# to use the sentences as input for trigram dtm, we clean the data
...
# we try the topic modelling for unigrams, bigrams and trigrams (substitute the first argument)
# Topics do not reveal reportable risks, so we decide to dig deeper
...
apply(topic_ai_sentences@beta, 1, function(x) head(topic_ai_sentences@terms[order(x, decreasing = T)],8))


# top negative terms in AI sentences according to LM Dictionary
...
# top negative trigrams from AI-related sentences
...
# we conduct a custom topic modelling on the trigrams
LM.neg.tri.topics <- LM.neg.sent.tri %>% 
  group_by(word) %>% 
  summarise(topic = paste(str_replace_all(trigram, "_", " "), collapse = " "),
            freq = sum(freq))

# the get_topics function extracts all words that occur in a trigram with the respective
# negative word from the LM Dictionary
LM.neg.tri.topics <- LM.neg.tri.topics %>% 
  mutate(topic = map(topic, get_topics))

# other than losses, however, the topics are not very promising
LM.neg.tri.topics %>% 
  arrange(desc(freq)) %>% 
  head(10) %>% 
  pull(topic)
...
# For the last approach, we try to run topic modelling on all sentences that contain
# negative words according to the LM dictionary from the trigrams

# extract all unique negative words (LM) that occur in trigrams 
negative_words <- LM.neg.sent.tri$word %>% unique()

# extract only sentences that contain these negative words
# paste sentences that contain the same negative word together into one document
# each negative word corresponds to one "document id" and all sentences that contain
# this word pasted together form the document
LM.neg.word.docs <- c()
for (word in negative_words) {
  ...
  LM.neg.word.docs <- append(LM.neg.word.docs, sentences)
}

# Make dtm of the resulting corpus
dtm_neg_word_docs <- LM.neg.word.docs %>% 
  VectorSource() %>% 
  Corpus() %>% 
  DocumentTermMatrix(control = list( 
    removePunctuation = T,
...
dtm_neg_word_docs$dimnames$Docs <- negative_words

# run the topic modelling on the resulting dtm
topic_neg_word_docs <- LDA(dtm_neg_word_docs,  # dtm
                  k = 12, # specifify number of topics (12 worked best)
                  method = "Gibbs",
                  control = list(
...
# display top 8 terms per topic for all 12 topics
apply(topic_test@beta, 1, function(x) head(topic_test@terms[order(x, decreasing = T)],8))

# log of probability reported
beta <- exp(topic_test@beta)
# display top 8 terms for only a single topic
